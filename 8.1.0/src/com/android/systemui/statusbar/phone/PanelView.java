...
public abstract class PanelView extends FrameLayout {
    ...
    private void notifyExpandingStarted() {
        log("notifyExpandingStarted,mExpanding="+mExpanding); 
        if (!mExpanding) {
            mExpanding = true;
            onExpandingStarted();
        }
    ...
    protected final void notifyExpandingFinished() {
        log("notifyExpandingFinished,mExpanding="+mExpanding);
        endClosing();
        if (mExpanding) {
            mExpanding = false;
            onExpandingFinished();
    ...
    private void runPeekAnimation(long duration, float peekHeight, boolean collapseWhenFinished) {
        log("runPeekAnimation,duration="+duration+",peekHeight="+peekHeight+",collapseWhenFinished="+collapseWhenFinished);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        log("onTouchEvent");
        RuntimeException panelViewonTouchEvent = new RuntimeException("panelViewonTouchEvent");
        panelViewonTouchEvent.fillInStackTrace();
        Log.w(TAG, "Called: " , panelViewonTouchEvent);
    ...
    private void endMotionEvent(MotionEvent event, float x, float y, boolean forceCancel) {
        log("endMotionEvent,x="+x+",y="+y+",forceCancel="+forceCancel);
        mTrackingPointer = -1;
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                if (mStatusBar.getBarState() == StatusBarState.KEYGUARD) {//锁屏界面，通知面板未下拉时，只能单手指解锁，多点触控的时候，拦截速度的计算，不响应事件
                    mMotionAborted = true;
                    if (mVelocityTracker != null) {
                        mVelocityTracker.recycle();
                        mVelocityTracker = null;
    ...
    protected void fling(float vel, boolean expand, float collapseSpeedUpFactor,
        ...
        log("fling,vel="+vel+",expand="+expand+",collapseSpeedUpFator="+collapseSpeedUpFactor+",expandBecasuseOfFalsing="+expandBecauseOfFalsing);
        cancelPeek();
    ...
    public void setExpandedHeight(float height) {
        if (DEBUG) logf("setExheightheigheightpandedHeight(%.1f)", height);
        log("setExpandedHeight,height="+height);
    ...
    public void setExpandedFraction(float frac) {
        log("setExpandedFraction frac="+frac);
        setExpandedHeight(getMaxPanelHeight() * frac);
    }

    public float getExpandedHeight() {
        log("getExpandedHeight mExpandedHeight="+mExpandedHeight);
        return mExpandedHeight;
    }

    public float getExpandedFraction() {
        log("getExpandedFraction mExpandedFraction="+mExpandedFraction);
        return mExpandedFraction;
    }

    public boolean isFullyExpanded() {
        log("isFullyExpanded mExpandedHeight="+mExpandedHeight+",getMaxPanelHeight()="+getMaxPanelHeight());
        return mExpandedHeight >= getMaxPanelHeight();
    }

    public boolean isFullyCollapsed() {
        log("isFullyCollapsed mExpandedFraction="+mExpandedFraction);
        return mExpandedFraction <= 0.0f;
    }

    public boolean isCollapsing() {
        log("isCollapsing mClosing="+mClosing);
        return mClosing;
    }

    public boolean isTracking() {
        log("isTracking mTracking="+mTracking);
        return mTracking;
    }
    ...
    public void collapse(boolean delayed, float speedUpFactor) {
        ...
        log("collapse  delayed="+delayed+",speedUpFactor="+speedUpFactor);
    ...
    private void startUnlockHintAnimationPhase1(final Runnable onAnimationFinished) {
        log("startUnlockHintAnimationPhase1");
    ...
    private void startUnlockHintAnimationPhase2(final Runnable onAnimationFinished) {
        log("startUnlockHintAnimationPhase2");
    ...
    private ValueAnimator createHeightAnimator(float targetHeight) {
        log("createHeightAnimator  targetHeight="+targetHeight);
    ...
    protected void notifyBarPanelExpansionChanged() {
        log("notifyBarPanelExpansionChanged");
